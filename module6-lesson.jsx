...
async function assessWithClaude(systemPrompt, userMessage, maxTokens = 1024) {
  ...
  try {
    res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: ASSESS_MODEL, max_tokens: maxTokens, system: systemPrompt, messages: [{ role: "user", content: userMessage }] }),
    });
  } catch (e) {
...
const RECALL_SYSTEM = `You are an expert instructor assessing a student's free-recall answer for Module 6: "Debugging Failure Modes." The module covers three failure types (Routing Failure, Output Drift, Pipeline Break), their symptoms and diagnostic targets, and a 7-step debugging checklist.
...
const MASTERY_SYSTEM = `You are an expert instructor writing an end-of-module mastery report for a student who has just completed Module 6 ("Debugging Failure Modes") of a course on Agent-Readable Skills Infrastructure. The module covers: the three skill failure types (Routing Failure, Output Drift, Pipeline Break) and their distinguishing symptoms; how to diagnose each (especially routing failures); what output drift is and its root cause; and the 7-step production debugging checklist.
...
function SectionComplete({ recallScores, quizScores }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const msg = `MODULE 6 — Debugging Failure Modes\n\nOVERALL SCORE: ${pct}%\n\nRECALL RESULTS:\n${recallSummary}\n\nQUIZ RESULTS:\n${quizSummary}`;
