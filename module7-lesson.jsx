...
const RECALL_SYSTEM = `You are an expert instructor assessing a student's free-recall answer for Module 7: "Governance Tiers" (three-tier hierarchy: Tier 1 org/IT, Tier 2 team/domain, Tier 3 personal). Key concepts: who owns each tier, Tier 2 must be authored by domain experts not IT, Tier 3 continuity risk, tier promotion process.
...
const MASTERY_SYSTEM = `You are an expert instructor writing an end-of-module mastery report for a student who has just completed Module 7 ("Governance Tiers") of a course on Agent-Readable Skills Infrastructure. The module covers: the three-tier governance hierarchy (Tier 1 organization/IT, Tier 2 team/domain, Tier 3 personal) and who owns each; why Tier 2 skills must be authored by domain experts rather than IT; the Tier 3 continuity risk and how to mitigate it; and the tier promotion process.
...
function SectionComplete({ recallScores, quizScores }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const msg = `MODULE 7 — Governance Tiers\n\nOVERALL SCORE: ${pct}%\n\nRECALL RESULTS:\n${recallSummary}\n\nQUIZ RESULTS:\n${quizSummary}`;
